...
the library gives an convenient interface for the RPC methods. See the [JavaScript API](https://github.com/symverse-lab/Document/blob/master/Doc-APIs/JSON_RPC_API_github.md) for more. 
...
`Object` -  information about the sct contract. 
...
```js
//Request
curl -X POST -H "Content-Type: application/json; charset=utf-8" --data '{"jsonrpc":"2.0","method":"sct_getContract","params": "0x0ced1024eed02b234df2", "latest"],"id":1}' http://127.0.0.1:8545
...
3. `QUANTITY|TAG` - Integer of a block number, or the string `"earliest"` or `"latest"`

##### Returns

`Object` - Created item with SCT30 or SCT40
...
```js
//Request
curl -X POST -H "Content-Type: application/json; charset=utf-8" --data '{"jsonrpc":"2.0","method":"sct_getContractItem","params":["0x0ced1024eed02b234df2","1", "latest"],"id":1}' http://127.0.0.1:8545
...
`QUANTITY` - either balance (if it is SCT 20) or index of item(if it is SCT 30,40)

##### Example

```js
//Request
curl -X POST -H "Content-Type: application/json; charset=utf-8" --data '{"jsonrpc":"2.0","method":"sct_getContractAccount","params":["0x0ced1024eed02b234df2", "0x00000000000000000001", "latest"],"id":1}' http://127.0.0.1:8545

//Result SCT20 Account
{
    "jsonrpc":"2.0",
    "id":1,
    "result":{
...
Retrun allowed balance or allowed index of item depends on the contract type.
...
4. `QUANTITY|TAG` - Integer of a block number, or the string `"earliest"` or `"latest"`

##### Returns

`QUANTITY` - either allowed balance (if it is SCT 20) or index of allowed item (if it is SCT 30,40)

##### Example

```js
//Request
curl -X POST -H "Content-Type: application/json; charset=utf-8" --data '{"jsonrpc":"2.0","method":"sct_getAllowance","params":["0x0ced1024eed02b234df2", "0x00000000000000000001", "0x00000000000000000002", "latest"],"id":1}' http://127.0.0.1:8545
...
```js
//Request
curl -X POST -H "Content-Type: application/json; charset=utf-8" --data '{"jsonrpc":"2.0","method":"sct_getContractItemsByCategory","params":["0x0ced1024eed02b234df2","1", "latest"],"id":1}' http://127.0.0.1:8545
